...
function testCapabilityMatching() {
  ...
  // Test that agent with superset of capabilities can handle task
  const fullStackAgent = mockAgents.find(a => a.id === 'agent-3');
  const pythonTask = mockTasks.find(t => t.id === 'task-1');
  
  if (fullStackAgent && pythonTask) {
    const canHandle = canAgentHandleTask(fullStackAgent.capabilities, pythonTask.required_capabilities);
    console.log(`Can Full Stack Developer handle Python Code Review task? ${canHandle ? 'Yes' : 'No'}`);
  }
